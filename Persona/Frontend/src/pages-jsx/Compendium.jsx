...
function PersonaImage({ persona }) {
  const imagePath = persona?.id ? PERSONA_IMAGES[persona.id] : null;
  ...
  return (
    <img
      ...
      onError={(event) => {
        ...
        // cannot be loaded.
        if (
          event.currentTarget.src.endsWith("/images/default-placeholder.png")
        ) {
          return;
        }

        event.currentTarget.src = "/images/default-placeholder.png";
      }}
    />
  );
}
...
export default function Compendium() {  
    ...
    async function loadPersonas() {
      try {
        ...
        if (!Array.isArray(data)) {
          throw new Error("The Persona API returned invalid data.");
        }
        ...
        if (!cancelled) {
          ...
          setError(err.message || "Failed to load Persona database.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      const exactMatch = personas.find(
        (persona) =>
          String(persona.name || "").toLowerCase() === query.toLowerCase(),
      );

      if (exactMatch) {
        setSelectedPersona(exactMatch);

        return;
      }

      // Partial name match
      const partialMatch = personas.find((persona) =>
        String(persona.name || "")
          .toLowerCase()
          .includes(query.toLowerCase()),
      );

      setSelectedPersona(partialMatch || personas[0]);
    ...
    setSelectedPersona((current) => {
      if (current && personas.some((persona) => persona.id === current.id)) {
        return current;
      }
  ...
  const filteredPersonas = useMemo(() => {
    ...
    return personas.filter((persona) => {
      ...
      const matchesGame =
        selectedGame === "All" || persona.originGame === selectedGame;

      return matchesSearch && matchesGame;
    });
  }, [personas, searchTerm, selectedGame]);
  ...
  const gameOptions = useMemo(() => {
    ...
    personas.forEach((persona) => {
      if (persona.originGame) {
        games.add(persona.originGame);
      }
    });
  ...
  const formatElementLabel = (element) => {
    ...
    if (key === "curse" || key === "dark") {
      return "CURSE / DARK";
    }

    if (key === "bless" || key === "light") {
      return "BLESS / LIGHT";
    }
  ...
  const activeAffinities = selectedPersona
    ? Object.entries(selectedPersona.affinities || {}).filter(
        ([, type]) =>
          type !== "-" &&
          type !== null &&
          type !== undefined &&
  ...
  if (loading) {
    return (
      <div className="compendium-page">
        ...
        <div className="compendium-loading">Loading Persona records...</div>
      </div>
    );
  }
    ...
    return (
      <div className="compendium-page">
        ...
        <div className="compendium-error">
          <p>Unable to load the Persona database.</p>
              ...
              filteredPersonas.map((persona) => (
                <button
                  ...
                  className={`persona-list-item ${
                    selectedPersona?.id === persona.id ? "active" : ""
                  }`}
            ...
            <div className="compendium-block">
              ...
              {activeAffinities.length > 0 ? (
                <div className="affinity-table">
                  {activeAffinities.map(([element, type]) => (
                    <div
                      key={element}
                      className={`affinity-cell ${String(type).toLowerCase()}`}
                    >
