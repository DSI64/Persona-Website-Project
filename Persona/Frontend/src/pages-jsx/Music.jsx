...
const getNextValidIndex = (tracks, startIndex, direction) => {
  ...
  while (!tracks[idx]?.embedId && count < len) {
...
export default function Music() {
    ...
    async function loadMusic() {
      try {
        ...
        if (!Array.isArray(data)) {
          throw new Error("Music API returned invalid data.");
        }

        const playableTracks = [...data]
          .filter(
            (track) =>
              track && track.embedId && String(track.embedId).trim() !== "",
          )
          .sort((a, b) => {
  ...
  // Keeps current track index reference synchronized
  useEffect(() => {
  ...
  // ============================================================
  // KEEP EVENT-HANDLER REFS FRESH so it doesn't break
  // ============================================================

  useEffect(() => {
    randomizeRef.current = randomizeOnFinish;
  }, [randomizeOnFinish]);
  ...
  // Loads song when current track finishes / changes
  useEffect(() => {
    ...
    try {
      if (isPlayingRef.current) {
        playerRef.current.loadVideoById(String(currentTrack.embedId));
      } else {
        playerRef.current.cueVideoById(String(currentTrack.embedId));
      }
    } catch (err) {
  ...
  const handleNext = () => {
    ...
    setIsPlaying(true);
    setCurrentTime(0);
    setCurrentIndex((previous) => getNextValidIndex(tracks, previous + 1, 1));
  };
  ...
  // Instant Randomize Handler
  const handleRandomize = () => {
    const validIndices = tracks
      .map((track, index) => (track.embedId ? index : null))
  ...
  const handleTrackSelect = (track) => {
    const index = tracks.findIndex(
      (item) =>
        item.id === track.id ||
        (item.title === track.title && item.game === track.game),
    );
  ...
  const onPlayerReady = (event) => {
    ...
    try {
      ...
      console.error("YouTube ready error:", err);
    }
  };
  ...
  // Youtube Options (When music player connects to youtube for the song)
  const ytOptions = {
    height: "10",
    width: "10",
  ...
  // Song Progress for duration bar (Calculates bar for song)
    ...
    return (
      <div className="music-page-container">
        ...
        <header className="music-header">
          ...
          <p>Loading soundtrack records...</p>
        </header>
      </div>
    );
  }
  ...
  // Empty State (When Backend doesn't connect)
  if (!currentTrack) {
    return (
      <div className="music-page-container">
        <div className="landing-grain"></div>
        ...
        <header className="music-header">
          <div className="eyebrow">Velvet Room Audio Logs</div>
          <h2>Soundtrack Compendium</h2>
          <p>No playable tracks were found.</p>
        </header>
      </div>
    );
  }
            ...
            <div className="category-list">
              {categories.map((category) => {
                ...
                const categoryTracks = tracks.filter(
                  (track) => category === "All" || track.category === category,
                );
          ...
          <div className="music-player-card">            
            ...
            <div className="player-controls-area">
              ...
              <div className="track-meta-header">
                <h3>{String(currentTrack.title).replace(/(\(|-)/, "\n$1")}</h3>
                <p>
                  {currentTrack.game} // {currentTrack.artist}
                </p>
              </div>
